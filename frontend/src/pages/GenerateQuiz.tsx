...
import { previewArticle } from "../api/quizApi";
...
export default function GenerateQuiz() {
  ...
  const handlePreview = async () => {
    ...
    if (!url.includes("wikipedia.org/wiki/")) {
      setPreviewError("Please enter a valid Wikipedia article URL");
      return;
    }

    try {
      const res = await previewArticle(url);
      setPreviewTitle(res.title);
    } catch {
      setPreviewError("Unable to fetch article preview");
    }
  };
      ...
      <div className="flex gap-3 mb-4">
        ...
        {previewTitle && (
          <p className="mt-2 text-sm text-green-600">
            📄 Article detected: <b>{previewTitle}</b>
          </p>
        )}
